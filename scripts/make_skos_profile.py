...
# original ont graph
g = rdflib.Graph().parse("../geoadminfeatures.ttl", format="turtle")
...
g_skos.bind('geoad', rdflib.Namespace("https://linked.data.gov.au/def/geoadminfeatures#"))
...
g_skos.remove((
    rdflib.URIRef("https://linked.data.gov.au/def/geoadminfeatures"),
...
g_skos.add((
    rdflib.URIRef("https://linked.data.gov.au/def/geoadminfeatures"),
    rdflib.namespace.SKOS.prefLabel,
    rdflib.Literal("GeoAdmin Feature Types", lang='en')
))
...
g_skos.add((
    ...
    rdflib.namespace.SKOS.definition,
    rdflib.Literal(
        "This vocabulary lists types of geological administrative features relevant to the duties of the Geological "
        "Survey of Queensland. It is automatically derived from GSQ's Geological Administrative Features Ontology "
        "(https://linked.data.gov.au/def/geoadminfeatures)", lang='en')
))
...
#   Ontology-specific
#
# add in a hasTopConcept for Admininstrative Feature since the generic hastopConcept adder doesn't pick it up
# since it has it's own subClassOf link (to geo:Feature)
g_skos.add((
    rdflib.URIRef("https://linked.data.gov.au/def/geoadminfeatures"),
    rdflib.namespace.SKOS.hasTopConcept,
    rdflib.URIRef("https://linked.data.gov.au/def/geoadminfeatures#AdministrativeFeature")
))

with open("../geoadmin-feature-types.ttl", "w") as f:
